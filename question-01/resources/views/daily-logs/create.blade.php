...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="mt-2 text-slate-500">Catat aktivitas harian Anda untuk direview oleh atasan</p>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-100 bg-slate-50">
            <div class="flex items-center space-x-3">
                <div class="w-10 h-10 rounded-xl gradient-bg flex items-center justify-center shadow-md shadow-indigo-500/20">
                    <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                    </svg>
                </div>
                <div>
                    <h3 class="text-lg font-semibold text-slate-900">Form Log Baru</h3>
                    <p class="text-sm text-slate-500">Isi detail aktivitas Anda</p>
                </div>
            </div>
        </div>
        ...
        <form action="{{ route('daily-logs.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <textarea name="activity" id="activity" rows="8"
                          placeholder="Jelaskan aktivitas yang Anda lakukan hari ini dengan detail...
...
- dll."
                          class="block w-full py-3 px-4 rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all text-sm resize-none @error('activity') border-red-300 bg-red-50 @enderror">{{ old('activity') }}</textarea>
                @error('activity')
                <p class="mt-2 text-sm text-red-600 flex items-center">
                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            ...
            <div class="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                <div class="flex">
                    ...
                    <div class="ml-3">
                        <h4 class="text-sm font-medium text-indigo-800">Informasi</h4>
                        <p class="mt-1 text-sm text-indigo-700">
                            Log yang Anda submit akan dikirim ke atasan langsung untuk diverifikasi. Pastikan aktivitas yang dicatat sudah sesuai.
                        </p>
                    </div>
                </div>
            </div>
